...
ifneq ($(CIRCLECI),)
	GO_PACKAGES=$(shell go list ./... | circleci tests split)
endif
...
pkg/install:
	$(call target)
	GO111MODULE=on CGO_ENABLED=$(CGO_ENABLED) GOOS=$(GO_OS) GOARCH=$(GO_ARCH) go install -v $(shell go list -f '{{if and (or .GoFiles .CgoFiles) (ne .Name "main")}}{{.ImportPath}}{{end}}' ${PKG}/...)

## test, bench and coverage

.PHONY: test
test: CGO_ENABLED=1  # needs race test
test: GO_FLAGS+=${GO_TEST_FLAGS}
...
.PHONY: coverage
coverage: CGO_ENABLED=1  # needs race test
...
coverage:  ## Takes packages test coverage.
	$(call target)
	GO111MODULE=on CGO_ENABLED=$(CGO_ENABLED) $(GO_TEST) -v -race $(strip $(GO_FLAGS)) -covermode=atomic -coverpkg=$(PKG)/... -coverprofile=coverage.out $(GO_PACKAGES)
...
.PHONY: coverage/ci
coverage/ci: CGO_ENABLED=1  # needs race test
...
coverage/ci:  ## Takes packages test coverage, and output coverage results to CI artifacts.
	...
	GO111MODULE=on CGO_ENABLED=$(CGO_ENABLED) $(GO_TEST) -a -v -race $(strip $(GO_FLAGS)) -covermode=atomic -coverpkg=$(PKG)/... -coverprofile=/tmp/ci/artifacts/coverage.out $(GO_PACKAGES) 2>&1 | tee /dev/stderr | go-junit-report -set-exit-code > /tmp/ci/test-results/junit.xml
...
.PHONY: mod/install
mod/install: GO_PACKAGES=${GO_VENDOR_PKGS}
mod/install: mod/tidy mod/vendor
mod/install:  ## Install the module vendor package as an object file.
	$(call target)
	@GO111MODULE=off go install -v $(strip $(GO_FLAGS)) $(GO_PACKAGES) || GO111MODULE=on go install -mod=vendor -v $(strip $(GO_FLAGS)) $(GO_PACKAGES)
